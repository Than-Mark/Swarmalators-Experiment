...
if __name__ == '__main__':
    ...
    alpha=1
    epsilon = 1
    # K = np.array([3.5 - 0.02 * i for i in range(101)])
    K = np.array([0.5 + 0.05*i for i in range(61)])
    fre1 = np.random.normal(0, 1, N)
    fre2 = -fre1
    fre1 = np.array(fre1)
    fre2 = np.array(fre2)
    ...
    np.save('person=1自然频率非爆炸a=1前向1层细致序参量.npy', R1)
    np.save('person=1自然频率非爆炸a=1前向2层细致序参量.npy', R2)
    plt.show()